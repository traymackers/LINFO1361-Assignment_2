...
class VisualGameManager:
    ...
    def __init__(
        self, red_agent=None, black_agent=None, total_time=300, min_agent_play_time=0.5
    ):
        """
        ...
        self.screen = pygame.display.set_mode(
            (70 * self.dim[1] + 100, 70 * self.dim[0] + 150)
        )
    ...
    def _handle_mouse_click(self, pos):
        ...
        if len(self.selected_actions) == 0 and self.human_to_play:
            ...
            if (row, col) in start_action:
                self.selected_actions = [
                    action for action in self.actions if action[0] == (row, col)
                ]
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE and len(self.selected_actions) == 0:
                    ...
                    and len(self.selected_actions) > 0
                    ...
                    self.selected_id = (self.selected_id - 1) % len(
                        self.selected_actions
                    )
                elif (
    ...
    def _agent_thread(self):
        ...
        remaining_time = (
            ...
            else self.remaining_time_black
        )
    ...
    def _draw_board(self):
        for i in range(self.dim[0]):
            for j in range(self.dim[1]):
                pygame.draw.rect(
                    self.screen, "Black", (70 * j + 50, 70 * i + 50, 70, 70), 1
                )
    ...
    def _draw_pieces(self):
        if len(self.selected_actions) > 0:
            ...
            pygame.draw.circle(
                self.screen, "#81a2c5", (70 * start[1] + 85, 70 * start[0] + 85), 8
            )
            pygame.draw.circle(
                self.screen, "#5783b2", (70 * start[1] + 85, 70 * start[0] + 85), 5
            )
            pygame.draw.circle(
                self.screen, "#77d373", (70 * end[1] + 85, 70 * end[0] + 85), 8
            )
            pygame.draw.circle(
                self.screen, "#49c445", (70 * end[1] + 85, 70 * end[0] + 85), 5
            )
            ...
            if removed:
                for pos in removed:
                    pygame.draw.line(
                        self.screen,
                        "Red",
                        (70 * pos[1] + 55, 70 * pos[0] + 55),
                        (70 * pos[1] + 115, 70 * pos[0] + 115),
                        3,
                    )
                    pygame.draw.line(
                        self.screen,
                        "Red",
                        (70 * pos[1] + 115, 70 * pos[0] + 55),
                        (70 * pos[1] + 55, 70 * pos[0] + 115),
                        3,
                    )
                ...
                for j in range(self.state.dim[1]):
                    if (i, j) in self.state.pieces:
                        ...
                        if (i, j) in start_action and self.human_to_play:
                            pygame.draw.circle(
                                self.screen, "#81a2c5", (70 * j + 85, 70 * i + 85), 8
                            )
                            pygame.draw.circle(
                                self.screen, "#5783b2", (70 * j + 85, 70 * i + 85), 5
                            )
    ...
    def draw(self):
        ...
        if self.winner is not None:
            ...
            if self.winner == 0:
                ...
                text = self.win_font.render(
                    f"{'Red' if self.winner > 0 else 'Black'} wins!", True, "Black"
                )
            text_rect = text.get_rect(
                center=(self.screen.get_width() // 2, 70 * self.dim[0] + 100)
            )
            ...
            if self.human_to_play:
                text = self.number_font.render(
                    f"{'Red' if self.state.to_move() == 1 else 'Black'} to play",
                    True,
                    "Black",
                )
                text_rect = text.get_rect(
                    center=(self.screen.get_width() // 2, 70 * self.dim[0] + 75)
                )
                self.screen.blit(text, text_rect)
            else:
                text = self.number_font.render(
                    f"{'Red' if self.state.to_move() == 1 else 'Black'} AI is thinking...",
                    True,
                    "Black",
                )
                text_rect = text.get_rect(
            ...
            if len(self.selected_actions) > 0:
                text = self.number_font.render(
                    f"Action {self.selected_id+1}/{len(self.selected_actions)}",
                    True,
                    "Black",
                )
                text_rect = text.get_rect(
            ...
            remaining_red -= (
                (time.perf_counter_ns() - self.start_thinking_time) * 1e-9
                if self.state.to_move() == 1
                else 0
            )
            ...
            text_rect = text.get_rect(
                center=(self.screen.get_width() // 4, 70 * self.dim[0] + 125)
            )
            self.screen.blit(text, text_rect)
            text = self.number_font.render(
                f"Black: {remaining_black:.2f} s", True, "Black"
            )
            text_rect = text.get_rect(
                center=(3 * self.screen.get_width() // 4, 70 * self.dim[0] + 125)
            )
            self.screen.blit(text, text_rect)
...
if __name__ == "__main__":
    ...
    #currentAgent = past_agents.all_agents[3](player=1, depth=3)
